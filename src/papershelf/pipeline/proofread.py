...
- **块结构**（宿主 2026-09-15：「还要校对**分块**分得对不对，尽量不要把一句话拆分到两个段里」）：
  ...
  **不要动**。`merge_block` 只能并入**本页上一块**，正好就是这种情形。
  ...
  ⚠️ `type` 为 `deco` 的块（出版社/期刊的**矢量标识**，如 Springer 马标、"Check for updates"
  徽标）是**渲染用的装饰图**（解析阶段栅格化的透明 PNG）：没有文字、每页重复出现，
  但**既不删也不改、不许参与 reorder 之外的任何操作**（它们不是内容，也不该被合进正文）。
  同理，带 `底纹` 标记的块（背后有一条浅灰色带，如 `CRITICAL REVIEW`）**保持原样**。
...
class ProofreadTools:
    ...
    def _t_read_blocks(self, a: dict) -> ToolOut:
        ...
        for b in bs[off:off + lim]:
            ...
            if b.type == "deco":
                # 出版社/期刊的矢量标识（v11）：每页重复出现，但那是**原件的版面内容**
                item["note"] = ("出版社/期刊标识（矢量图形栅格化成的装饰图）：没有文字、"
                                "不要删除、不要改动、不要并入别块。")
