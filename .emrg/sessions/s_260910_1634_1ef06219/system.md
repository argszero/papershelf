...
### 最近一轮（2026-09-19）㊹ 修订：参考文献「只翻译了两条」= 连号判据钉死起点 1（解析 v14）

宿主：「生产环境，References 只翻译了两条，剩下的既没有翻译，也无法重译」（截图两张：
`reader/1` 参考文献区 `[1]`/`[2]` 有中文、`[3]` 往后全英文）。**新会话无 ssh（生产 22 端口封）**：
用**只读**生产块 dump + 宿主已登录标签页 `fetch('/api/papers/1/doc')` 取证（零改动），
本地 `ps_ro.db` 副本与线上一致（820 块 / `ref` 2 / `refs` 309）。
- **病灶**：`_ref_entry_starts` 的**连号起点钉死在 1** ⇒ 只有恰好从 `[1]` 起的段能切；
  而**跨页页眉/页脚（`band`）把文献流截成 6 段** ⇒ **只有第一段成功**，后 5 段每片都带连号
  `[N]` 却一律回 0 起点（实测候选 53/56/62/51/52/12 个）。碎片留在免中文的 `refs` ⇒ 无译文、无重译入口。
- **修法**：`_longest_consecutive_run` —— 取**最长连号串**（起点不限），形状认错的候选**跳过而不中断**。
  `29516b9`（代码+测试+docs）；**变异检验换回旧实现 4 条全红**；离线回归 **464 passed**。
- **实测**（生产真实碎片，零 token）：paper1 `2→288`、paper3 `9→120`、paper4 `28→177` 条；
  逐段**字符守恒**；抽样 6 条真条目过真 LLM → 只译标题、作者/DOI 逐字保留、`needs_review` 0
  （6,002 tokens/6 条）；**合成两页稿走真产品路径**（页眉页脚截断）→ 6 条全 `ref`＋中文；
  真浏览器三模式逐列量测通过。
...
- **四件事一起错**（没有一件是"渲染丢了"）：`" ".join(text.split())` 把 6 行拼成一段 /
  整块字号比正文小这个**事实没记** / 行首**上标小标**被拉平 / 注区**上方**那条页中横线没记
  （`_rule_marks` 的窗口只看页顶页底 14%，这条在 **81%**）。
- **不拆块**（宿主选 A）：块文本按源行拼 + `white-space: pre-line`；三枚戳 `note`（`--note-fs`）/
  `markers`（与行平行的行首标记 → `<sup>`）/ `rule`（页中横线，与页边横线同一份 `_rule_spec`）。
  判据两条缺一不可（≥2 行以上标小标起头 **且** 整块主体字号比正文小 1pt 以上）。`PARSE_VERSION` → **16**。
- **红线：批注坐标一个都不动** —— 真机量过：拖选注区里的 `optical`，DOM 尺子算 `[150,157)`，
  库里 `en.index("optical")` **也是 150**；`textContent.length 581` = 库里 `len(en)`。
- **存量出口 `notesfix`（绝不「重新提取」）**：配块判据 = 同页 + 去空白逐字相同；只改写这几个块
  ⇒ 别的块的批注**数学上不可能变**；中文侧重译 ⇒ 划痕删/笔记降文献级（**留住内容**）；
  **人工修订块不重译**（⑯）；**只翻被改的那几块**；块 + 批注同一次 `tx`。
- ⚠️ **途中抓到一个真缺陷**：注上横线的 `side` 第一版写成 `"below"` ⇒ 线画到注区**下面**去了
  （宿主要的是"上方那条线"）。**旧断言写的就是缺陷本身**（`assert … == "below"`），
  所以"改回 below"的变异最初是**绿的** —— 变异检验这次抓的是"测试与实现同源"。
...
### 最近一轮（2026-09-20）㊹ 修订三：APA 参考文献判据（解析 v15）+ 生产两次定点重建 + 真浏览器验收

宿主授权（本会话）：**「你上生产就行」** + **「生产操作也可做，前提是笔记/高亮不丢不乱」**
（＝唯一的硬不变量）。细节全在 `design-decisions.md` **㊹ 修订三**。

- **`6708ed6`**：第三条判据 `_author_year_starts`，只在编号判据失效时启用；`PARSE_VERSION` → **15**；
  paper 2（APA，182 碎片）→ **163 条**，逐字守恒。离线 **516 passed**（`tests/test_refs.py` +8）、
  **8 处变异全红**。CI `35480550066` 六 job 全绿 → 生产 `revision=6708ed6…` / healthy / health 200
  （DB 备份 `/tmp/papershelf.db.bak.20260920-0915`）。
- **生产定点重建**（走 ㊹ 修订二 的「重建参考文献」，**不是**「重新提取」——后者会 `DELETE notes/highlights`）：
  paper 3 `ref 9→120`（134,252 tokens / 待校对 2）、paper 2 `ref 0→163`（202,730 tokens / 待校对 0）。
  **每次前后实测：`notes 19→19` / `highlights 433→433` 全字段一致**。
- **真浏览器验收**（宿主已登录标签页 + 真实鼠标）：reader/2 中文栏 **163/163 含 CJK**、英文栏 0/163、
  `.ref-item` 335 个（163×2 + 9×1）；reader/3 **120/120**；三模式 `en 368 / zh 345 ↔ 0/368 ↔ 368/0`
  并还原 dual；`b-0951` 中文标题上真实拖选 → `.mark-bar` 出现 → `Esc` 收起（**只拖不写**）；
  零 console error、零横向溢出；验收完 marks 再比对仍**逐字段一致**。
- **一条被证伪的疑似缺陷**：`b-1111`/`b-1112` 看着"全是英文" ≠ 没译 —— 决策 B **只译标题**，
  作者串本来就保原文；两条的中文栏里都有中文标题。**判据要落在「中文栏里有没有 CJK」**。
- **⚠️ 新边界（留给宿主）**：跨页家具把 **6 条**条目的尾巴切在 `refs` 里（paper 2 ×4 / paper 1 ×1 /
  paper 4 ×1），文字一个字没丢但被切成两段、尾巴没中文；修法需「跨家具续接」，**未做**。
  `refs` 里真正的页面家具（页码/页眉）保留是**有意的**。
- **㊻ 存量回填**：按上轮盘点（29 处中文全读得通）**建议不做**；是否回填仍待宿主一句话。

### 最近一轮（2026-09-20 下午）㊼ 手工修订块 = **三个原语**：插入 / 编辑 / 删除 —— 已上生产 `8f988e0`

宿主先要「手动合并/拆分段落」，随即自己把设计收敛掉：**拆分 = 后插一块 + 编辑原块剪掉后半；
合并 = 编辑前一块接上 + 删掉空掉那块** —— 所以不做 `split`/`merge` 两个复合动作，
只做**新块插入 / 老块编辑 / 老块删除**。价值在于「**批注不丢不乱**」从一句期望变成**可证明的性质**：
只要程序不提供复合动作，就没有任何一步需要"猜这两段其实是一句话"。细节全在 `design-decisions.md` **㊼**。

- **新** `server/blockops.py`：`remap_span`（最长公共前缀+后缀的**确定性**重映射；落进被替换段内
  **返回 `None`** —— 绝不硬贴到邻字）+ `insert_block`/`edit_block`/`delete_block`，全部走
  `refsfix.py::rebuild_paper_refs` 的「块改写 + 批注搬家**同一次 `tx`**」范式（`db.tx` 不可重入）。
- **三条不变量**：① 只碰被改那块（`WHERE paper_id=? AND id=?`；新块 id = 现有最大号 + 1，
  **已有块 id 永不重编号** ⇒ 别的块上的 `(block_id,lang,start,end)` **数学上不可能变**）；
  ② 坐标不猜（映射不了 ⇒ 划痕删 / 笔记**内容保留**、锚点清空成「文献级笔记」，当场报给用户）；
  ③ 删块**不删笔记**（`content` 比锚点值钱）、`hl_id` 置 NULL。另：两栏**各自坐标系**重锚。
- **前端硬规则**：`saveBlock` **只发真正变了的字段** —— 否则「开抽屉再点保存」这一下就会把整块
  机器译文永久标成 `zh_source='human'`（⑯ 的正确语义被误触）。一个字段都没变 ⇒ 提示「没有改动」。
- **实测**：离线 **548 passed**（新 `tests/test_blockops.py` 32 项，含「相邻块批注逐字段零变化」+
  前后端 `TEXT_TYPES` 一致性）；**7 处变异全红**（其中 ① 抓到一个**真缺陷**：`sets`/`args` 分开攒
  ⇒ 部分列改时错位，`payload` 列拿到 `"human"` 且不报错）；本地 8012 真浏览器真实鼠标逐动作实测
  （插入 `b-0043` 落在 `b-0021`/`b-0022` 之间 / 编辑报「2 处批注已随文字移动」/ 破坏性剪裁报
  「1 处划痕移除 · 1 条笔记转为文献级（内容保留）」/ 删除后相邻块 identical）。
  ⚠️ 验收中**误删了一个正文块**（工具条挂在"当前选中的块"上，长文档里选中项会随点别处移动）
  → 已按 09-17 备份**逐字节还原**；两条教训：删除前必读确认框里那行原文 / `window.confirm`
  在自动化里要桩成"记录文案 + 断言包含标记"，**不能无脑 true**。
- **上线**：`8f988e0` → CI six job 全绿 → 生产 `docker compose pull app && up -d app` →
  `revision=8f988e0e…` / healthy / 公网 200 / bundle `index-OeDqeWaX.js`（与本地同名）；
  生产 4 篇 **`notes 19→19` / `highlights 433→433` 逐字段 identical**、各篇块数不变
  `{1:811, 2:428, 3:429, 4:435}`；生产真浏览器 reader/3 工具条三原语俱全、删块按钮
  `oklch(0.58 0.19 25)`（正确红）。
- **边界**：`TEXT_TYPES = p / h2 / h3 / h4 / abstract / refs / ref`（`figure`/`table`/`deco`/`eq`
  的文字在 `payload` 里，凭空造只会渲染成空白；`h1` 归论文标题、改元数据）。新块**继承**
  `section`/`payload.page`（否则掉出 PDF 分页），**不继承** `band`/`rule`/`shade`/`seam`。
  ⚠️ **拆分时被剪掉那半上的批注不会跟着走进新块**（新块里的字是用户新粘的、没有坐标）——
  这与宿主那句「只影响当前 block 的笔记」一致，是**有意的**边界，不是缺陷。
  ⚠️ **本条已被 ㊽ 修订**（同日晚上）：`figure` 进了可编辑名单，名单由**一个拆成三个**
  （`TEXT_TYPES` / `NEW_TYPES` / `RETRANSLATE_TYPES`）—— 见下一节。

### 最近一轮（2026-09-20 晚）㊽ 图注块（`figure`）= 可编辑 / 可重译 —— 已上生产 `51f9df3`

宿主附生产 `reader/2` 截图（`Fig. 2` 那块工具条只有「↑ 上方插入 / ↓ 下方插入 / 加笔记 / 删除此块」）：

> 图片说明的 block，也应该支持"编辑此块"和"重译此块"

细节全在 `design-decisions.md` **㊽**。要点：

- **一个名单拆成三个**：`TEXT_TYPES`（文字在 `en`/`zh` 里 ⇒「编辑此块」，**+ `figure`** ——
  图注就是那两栏）/ `NEW_TYPES`（新建与改类型可选，**不含 `figure`** —— 图靠 `payload.src`：
  凭空造 = "有图注没图"的空壳，改成正文 = 图**从页面上消失**）/ `RETRANSLATE_TYPES`（可能有
  中文 ⇒「重译此块」，含 `table`/`ref`，**不含** `refs` 碎片）。前后端两份名单**逐字钉住**。
- **图注原文有两个记录位**：`en`（渲染/翻译判定）+ `payload["caption"]`（**①c 校对 agent 读它**）
  ⇒ 手改原文必须同步写 `caption`，否则"中文栏还显示改之前那句英文图注"（渲染回落旧 caption）。
  ⚠️ 图注**没有** `caption_zh`（那是**表格专有**）⇒ 重译只写 `zh`，`payload.caption` 是原文、不该动。
- **顺带修掉一个真缺陷**（原有代码，被这次需求照亮）：`translate_blocks` 跳过
  `zh_source='human'` 的块（⑯ 重跑不得覆盖人工修订），而「重译此块」正是用户**明说**要覆盖
  ⇒ todo 空 ⇒ 端点把**旧中文**回写、还把来源从 `human` **降级**成 `mt` ——
  表现是「**点了重译没变化，『已人工修订』标记却消失了**」。修法 = 本次请求的副本摘掉来源戳。
  **真浏览器已验**（置人工修订 → 真实鼠标点重译 → `zh` 换新、`zh_source=mt`、`caption` 未动）。
- 「**这一块没有可译文字**」（400）提到「未配置 LLM」（503）**之前** —— 前者与 LLM 无关。
- 离线 **556 passed**（`tests/test_blockops.py` +8）/ **7 处变异全红**；生产 `51f9df3`（CI
  `35510215499` 六 job 绿、52.86MB 慢层 ≈10 分钟）/ healthy / bundle **`index-CJ737AoP.js`**
  （与本地构建逐字节同名）；生产真浏览器：`b-0150` 工具条出「编辑此块 | 重译此块」、
  点开是「编辑图注 · b-0150」抽屉且**无类型下拉**、取消关掉**零写入**；
  `notes 20` / `highlights 460` 验收前后**逐项 identical**。
- **两条现场教训**：① `activate_tab(id)` 之后**仍要按打印出来的 `location.href` 行事**
  —— 这次打印出的还是本地标签，我没停手，`location.href=…` 于是**把宿主的本地 dev 标签导航
  到了生产页**（已还原）。生产验收要用**宿主已登录的生产标签**，别拿本地标签当跳板。
  ② 本地 dev 的 LLM 是**真池子**：验「重译此块」真花了一次调用（按备份恢复数据）。
...
_Last updated: 2026-09-20T21:05:00+08:00_
...
### 最近一轮（2026-09-20 下午）㊼ 手工修订块 = 插入 / 编辑 / 删除（拆分合并由用户组合）—— **已上生产 `8f988e0`**

宿主 2026-09-20 授权后一次做完：`server/blockops.py`（`remap_span` + 三原语，与批注搬家**同一 `tx`**）
+ 三个端点回完整块 + 前端块工具条四按钮与编辑抽屉 → 离线 **548 passed** / **7 处变异全红**
（抓到真缺陷：`sets`/`args` 错位把 `payload` 写成 `"human"` 且不报错）→ 本地 8012 真机逐动作验
（插入 / 编辑报「N 处批注已随文字移动」/ 破坏性剪裁报「划痕移除 · 笔记转文献级」/ 删除后相邻块 identical）
→ push → CI 六 job 绿 → 生产 `revision=8f988e0e…` / healthy，**生产 4 篇 `notes 19`/`highlights 433`
前后逐字段 identical**、块数不变，真浏览器 reader/3 工具条三原语俱全。
完整决策（三条不变量 / 边界 / 前端只发变更字段的防误触规则）见项目记忆 `design-decisions.md` **㊼**。

**本轮两条现场教训**：① **工具条挂在"当前选中的块"上，长文档里选中项会随点别处移动** ——
验收时以为选中的还是刚插入那块，对确认框点了确定，**误删一个正文块**（已按备份逐字节还原）⇒
删除前必须读确认框里那行原文；自动化里 `window.confirm` 要桩成"记录文案 + 断言包含标记"，不能无脑 `true`。
② 生产验收前**先按 bundle 指纹确认不是化石标签页**（`index-OeDqeWaX.js`），
并 `Emulation.setFocusEmulationEnabled(True)` 再动真实鼠标。

### 最近一轮（2026-09-20 晚）㊽ 图注块可「编辑此块」/「重译此块」—— 已上生产 `51f9df3`

宿主（附生产 `reader/2` 截图：「图片说明的 block，也应该支持『编辑此块』和『重译此块』」）。
本轮是接着上一轮交接做的收尾：**代码早已写好但未提交**，本会话补完「全量回归 → 真浏览器
验那条修复 → 提交 → CI → 上生产 → 生产真浏览器验收 → 记忆落档」整条链。
细节全在项目记忆 `design-decisions.md` **㊽** + `MEMORY.md` 的 ㊽ 段；这里只留会话内局部经验。

- **一个名单拆成三个**（`TEXT_TYPES` / `NEW_TYPES` / `RETRANSLATE_TYPES`），前后端**逐字钉住**
  （测试直接读 `Reader.tsx` 的字面量比对）。
- **图注原文两个记录位**：`en` + `payload["caption"]`（**①c 读后者**）⇒ 手改必须同步；
  ⚠️ 图注**没有** `caption_zh`（**表格专有**）⇒ 重译只写 `zh`，`payload.caption` 是原文、不动。
- **真缺陷（原有代码）**：重译跳过 `zh_source='human'` ⇒ 端点回写**旧中文**并把来源**降级**成
  `mt`（「点了没变化，『已人工修订』标记却消失了」）。修法 = 本次请求的副本摘掉来源戳。
- **实测**：离线 **556 passed**（`test_blockops.py` +8）/ **7 处变异全红**；生产真浏览器 `b-0150`
  工具条出「编辑此块 | 重译此块」、抽屉「编辑图注 · b-0150」且**无类型下拉**、取消关掉**零写入**；
  `notes 20` / `highlights 460` 逐项 identical（DB 备份 `/tmp/papershelf.db.bak.20260920-2025`）。
- **两条现场教训**（已并入项目记忆）：
  ① **`activate_tab(id)` 之后仍要按打印出来的 `location.href` 行事** —— 打印了不等于处理了；
     这次打印出的还是**本地**标签地址，我没停手，`location.href=…` 于是把宿主的**本地 dev 标签
     导航到了生产页**（事后已还原成 `127.0.0.1:8012/reader/2`）。⇒ 生产验收请用**宿主已登录的
     生产标签**，别拿本地标签当跳板（两个都会被动过）。
  ② 本地 dev 的 LLM 是**真池子**：验「重译此块」真花了一次调用（验完按备份恢复数据）。
- ⚠️ 生产 `blocks` 基线本轮**变了**（宿主在自己操作）：`notes 20`（1:11/2:2/4:7）、
  `highlights 460`（1:229/2:50/4:181）、`blocks {1:809, 2:422, 4:435}`，且部署窗口内 paper 5 转换完成
  （548 块）。以后比对**以当次实测值为基线**，别照抄上一轮的 19/433。
...
**Memory Hygiene** (PR #941, rant 2026-08-23T08:04:26 + PR #1067, rant 2026-08-28T22:12:16 — write-first self-review, digest-style reorganization):
...
- **Daily full history** (never compacted): `/Users/argszero/scm/github.com/argszero/papershelf/.emrg/sessions/s_260910_1634_1ef06219/history_260921.jsonl`
...
## Temp File Rules (PR #987, rant 2026-08-25T18:10:57)
...
## ⛔ 最高原则·永久（宿主 2026-08-21 10:35 确立）

**任何时候、任何实例，禁止编写、恢复或以任何形式引入「执行自动升级触发链」的测试用例、脚本或代码路径**——包括真实请求 GitHub releases API、真实读写 `~/.emrg/install/version.txt`、真实创建/写入 `emrg-upgrade` 会话、直接调用 `UpgradeManager.tick()` / daemon `_run_upgrade_session` 而未经完全隔离的测试。实证：长时间运行的 pytest 曾在后台真实执行自动升级逻辑，每 5 分钟一次真实请求 releases API、真实读取 `install/version.txt`、真实向 `emrg-upgrade` 会话写入降级 prompt，跨 daemon 重启、甚至在全部进程停止后依然持续写入。违反将污染升级会话、发出真实网络请求并干扰宿主环境。测试必须完全隔离这条链：releases API（httpx AsyncClient）、VERSION_FILE、SESSION_ID（`emrg-upgrade`）、run_session_cb 全部打桩；`tests/conftest.py` 的 `_guard_upgrade_hermeticity` 是兜底守卫，触发真实升级链的测试直接断言失败。此原则永久有效，不适用任何演化机制。MANIFESTO.md 第四条附则三 载有同款条文。