...
| **小字注区**（v16，2026-09-21） | 宿主（生产 `reader/2` 截图）：「这一段提取的不对」「**主要是样式不对**」。病灶 = 表注 / 脚注 / 作者单位这类**小字注区**被当正文段渲染，四件事一起错：① `" ".join(text.split())` 把 6 行**拼成一段**（PDF 里是 6 行）；② 整块字号比正文小（8.5pt / 10pt）这个**事实没记**（库里的块文本只有字，字号早丢了）；③ 行首那个**上标小标**（`aThese studies…`，间距是画出来的、不是字符）被拉平；④ 注区**上方**那条页中横线没记（`_rule_marks` 只在页顶/页底 14% 内找页眉线，这条在 81% 处）。**方案：不拆块** —— 块文本**按源行拼**（换行符留在文本里）+ 渲染端 `white-space: pre-line`；盖三枚戳：`payload["note"]={"em": 0.85}`（`--note-fs`，小字灰字）、`payload["markers"]=["","a",…]`（与行**平行**的行首标记，渲染成 `<sup>`）、`payload["rule"]`（页中横线，形状与页边横线同一份 `_rule_spec`）。判据两条**缺一不可**：≥2 行以**上标小标**起头（行首 span 是单字母/1–2 位数字且比同行主体字号小 1pt 以上）+ 整块主体字号比正文小 1pt 以上 —— 只靠①会把展示公式的续行（`2 e2`）认成注区（实测 3 处误报），只靠②会把所有脚注、页眉页脚、图注都抓进来。`PARSE_VERSION` → **16** |
...
| **参考文献条目**（㊹，v13，2026-09-17） | 宿主：「参考文献没有翻译」→ **只译文献标题**（作者名/期刊/卷期页/DOI 保原文：读者要靠它们检索）。要译标题先得**有一条完整的条目**：PDF 抽出来是一条连续文字流，被块检测按栏/行组切成 ~30 个碎片（实测生产那篇 29 片 / 6,189 字符），切口常落在**词中间**（`man-` + `ufacturing`），一块里还塞着下一条的前半截。⇒ `merge_ref_entries` 把碎片按**条目编号**切成整条，每条一个 **`ref` 块**（`refs` 从此只表示"切不出条目的碎片"，仍免中文）。切分判据：`[N] ` 与 `N. `+大写字母两种风格各试一遍、取命中多的；**必须连号**（读到第 1 条之后下一个必须是 2 —— 这一条才挡得住 DOI 里的 `… / 1. Oe. 4582548`）；拼接**只动空白**（非空白字符逐字守恒，有测试钉住），行末断词的连字符**不猜补**（`Laser-directed` 是反例，该不该合是看图那一步①c 的活）；切不出来（无编号/编号中途失效/单条 >2000 字）→ **原样返回**，不丢字也不产出假条目。`PARSE_VERSION` → **13**；⚠️ **v14（2026-09-19）修掉一处"只有第一段能合并"的判据缺陷**：`_ref_entry_starts` 原先把连号的**起点钉死在 1**，而跨页的页眉/页脚（`payload["band"]`）会把参考文献流**截成好几段** ⇒ **只有含 `[1]`/`[2]` 的第一段合并成功**，其余各段明明每片都带着连号 `[N]` 起点却一段都切不出来（生产 paper 1 实测：6 段里后 5 段的 bracket 候选 53/56/62/51/52/12 个，`_ref_entry_starts` 一律回 0）—— 界面表现正是宿主报的「只翻译了两条，剩下的既没有翻译，也无法重译」（`refs` 免中文）。改法 = **取最长的那一串连号**（起点不限，见 `_longest_consecutive_run`；形状判据认错的候选**跳过而不中断**这一串）。生产四篇实测：paper 1 `2 → 288` 条、paper 3 `9 → 120`、paper 4 `28 → 177`；**paper 2 仍未合并（0 条）—— 它是 APA 体例、压根没有条目编号**。`PARSE_VERSION` → **14**；**v15（2026-09-20）补上那条缺口：无编号（作者-年份 / APA 系）体例的条目起点判据** —— 上面两条判据都要求条目带编号，APA 的条目头却是 `…, & Yoon, B. (2020).`：判据 = **年份括号锚点 + 它前面的作者串**（`_author_year_starts` / `_RE_REF_AUTHOR`），**只在编号判据完全失效时才走**（编号体例的条目正文里也常出现 `(2019)` 这类引用，抢跑会把成段的条目切碎）。作者串要认三种真形状：小写介词（`de Oliveira` / `du Plessis` / `t Veld`，**白名单**而非「任意短词」——`pdf`/`the`/`x` 都会冒充）、多词姓（`Mamat Ibrahim`）、带音调的词首与后缀（`& Özel, T.` / `Conway, J. C., Jr.`）；无年份的 `(n.d.).` 也算条目头。**末尾必须是「姓 + 逗号 + 首字母」**，这条要求同时挡住了「往回吞上一条的尾巴」。认不出作者串的锚点**跳过而不中断**（那一条并进上一条的块，文字一个字不丢）。生产 paper 2 实测：**182 个碎片 → 163 条**、逐字守恒、条条都有作者串；`PARSE_VERSION` → **15** |
...
- 每片 prompt 注入：
  1. 全局术语表（⑫，计划级）
  2. 前后各 `CTX_NEIGHBORS=2` 个**正文**块上下文（跳过页眉/页脚/装饰等页面家具，㊻）
  3. **跨块接缝标注**（㊻）：疑似「同一句被排版切成两块」的相邻正文对 → 给出这句的**完整原文**
     与断点，要求"先把整句译好、再按断点切成两半"（两块仍是两块，`data-b` 与块数不变）
  4. 输出格式硬约束（保留 `data-b`、不译公式 LaTeX、**参考文献条目只译标题**、数字与引用编号原样）
- **块级重译**（⑯）：同一 prompt 只送 1 块 + 上下文（含上面的接缝标注）
