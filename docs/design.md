...
| **文末材料** | 参考文献 + 作者简介从 `REFERENCES` 小标题起标为 `refs`（含小写大写伪空格还原 `R EFERENCES → REFERENCES`）；出版社水印页脚（`Authorized licensed use limited to…`）直接剔除 |
...
- **免中文块标 `data-nt="1"`**（参考文献 / 公式）：它们在中文视图里**回落英文原文**，
...
- **免中文块不进切片**：参考文献（实测占正文 26%）与公式/符号块（另 12%）**根本不送模型** ——
...
- 每片 prompt 注入：
  ...
  3. 输出格式硬约束（保留 `data-b`、不译公式 LaTeX、不译参考文献条目原文、数字与引用编号原样）
...
- 参考文献 / 公式块 → **不要求**（`data-nt` 标记 + `refs`/`eq` 类型）
...
- **凡是有可选文字的块都有尺子**（2026-09-16 修）：**标题与参考文献条目也算可划区域**。
  ...
  ①服务端 `markup.render_block` 的标题/`refs` 分支改走 `prose()`；
